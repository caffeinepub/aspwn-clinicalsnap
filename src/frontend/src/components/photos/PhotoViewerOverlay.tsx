// Full-screen photo viewer with zoom/pan, annotation tools including stamps, view template display in header, and edit view template action

import { useState } from 'react';
import { useAppStore } from '@/lib/state/useAppStore';
import { uint8ArrayToObjectURL } from '@/lib/media/photoStorage';
import { Button } from '@/components/ui/button';
import { X, Pencil, ZoomIn, ZoomOut, RotateCcw, Edit } from 'lucide-react';
...
export function PhotoViewerOverlay() {
  const { photos, selectedPhotoId, selectPhoto, updatePhoto } = useAppStore();
  ...
  const handleSaveTemplate = async (template: string | undefined) => {
    ...
    try {
      await updatePhoto(photo.id, { viewTemplate: template });
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2">
            ...
            {photo.viewTemplate ? (
              <button
                ...
                className="bg-white/10 text-white text-sm px-3 py-1.5 rounded hover:bg-white/20 transition-colors flex items-center gap-2 touch-target"
              >
                {photo.viewTemplate}
      ...
      <ViewTemplateEditorDialog
        ...
        currentTemplate={photo.viewTemplate}
